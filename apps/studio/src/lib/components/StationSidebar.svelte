<script lang="ts">
    ...
    let filteredStations = $derived.by(() => {
        ...
        if (filter === "metro")
            result = result.filter((station) => station.type === "metro");
        if (filter === "light")
            result = result.filter((station) => station.type === "light");
        if (filter === "duplicate") {
            ...
            result = result.filter((station) => {
                const duplicateName =
                    (nameCounts.get(station.name.toLowerCase()) ?? 0) > 1;
                const duplicateGeo =
                    station.geo &&
                    (geoCounts.get(`${station.geo.lat},${station.geo.lng}`) ??
                        0) > 1;
    ...
    const visibleStations = $derived(
        filteredStations.slice(startIndex, startIndex + visibleCount),
    );
...
>
    <div
        class="flex shrink-0 flex-col gap-3 border-b border-sidebar-border p-3"
    >

<script lang="ts">
  ...
  const railRadarBaseUrl = $derived(
    $stationStore.mode === "local" ? "http://localhost:3000" : "https://railradar24.com",
  );
...
>
  ...
  <ScrollArea class="min-h-0 flex-1">
    <div class="flex flex-col gap-4 px-4 py-4">
      <div class="flex flex-col gap-1.5">
        ...
        <p class="text-xs text-muted-foreground">Display name shown in the app</p>
        <Input id="station-name" bind:value={name} placeholder="Station name" />
      </div>
      ...
      <div class="grid grid-cols-2 gap-3">
        <div class="flex flex-col gap-1.5">
          ...
          <p class="text-xs text-muted-foreground">North-south position</p>
          <Input
            id="station-lat"
            type="number"
            step="0.000001"
      ...
      <details class="border-t border-border pt-4" open>
        ...
        <WikipediaInfo
          ...
          currentCoordinates={lat && lng ? { lat: parseFloat(lat), lng: parseFloat(lng) } : null}
  ...
  <div class="grid grid-cols-[1fr_auto] gap-2 border-t border-border bg-card px-4 py-3">
    <Button disabled={!name.trim() || isSaving || !hasChanges} onclick={handleSave}>

<script lang="ts">
    ...
    const isPlacingStation = $derived(
        Boolean(selectedStation && !selectedStation.geo),
    );
    ...
    function makeGeojson() {
        return {
            ...
            features: stations
                .filter(
                    (station) =>
                        station.geo && station.id !== selectedStationId,
                )
                .map((station) => ({
    ...
    function syncMarker() {
        ...
        element.className =
            "size-5 cursor-grab rounded-full border-2 border-white shadow-lg";
    ...
    $effect(() => {
        if (!loaded && stations.length === 0 && selectedStationId === null)
            return;
        syncSource();
        syncMarker();
    });
    ...
    $effect(() => {
        if (map)
            map.getCanvas().style.cursor =
                isAddingStation || isPlacingStation ? "crosshair" : "";
    });
    ...
    onMount(() => {
        ...
        map.on("load", () => {
            ...
            map.on("mouseenter", LAYER_ID, () => {
                if (map && !isAddingStation && !isPlacingStation)

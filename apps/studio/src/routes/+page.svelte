<script lang="ts">
  ...
  const selectedStation = $derived(
    selectedStationId
      ? (stationState.stations.find((station) => station.id === selectedStationId) ?? null)
      : null,
  );
  const existingIds = $derived(
    new Map(stationState.stations.map((station) => [station.id, station.name])),
  );
  ...
  function reconcileContribution(stationId: string) {
    ...
    const current = stationState.stations.find((station) => station.id === stationId) ?? null;
  ...
  async function handleConfirmCreate(id: string, name: string) {
    ...
    try {
      isSaving = true;
      const station = await stationStore.createStation(name, { lat, lng }, { id });
      ...
      showToast(error instanceof Error ? error.message : "Failed to create station");
    } finally {
      isSaving = false;
    }
  }
  ...
  async function handleMarkerDragEnd(id: string, lat: number, lng: number) {
    const previousStation = stationState.stations.find((station) => station.id === id);
    ...
    try {
      ...
      showToast(error instanceof Error ? error.message : "Failed to update position");
    }
  }
  ...
  async function handleSetStationLocation(lat: number, lng: number) {
    ...
    try {
      const updated = await stationStore.updateStation(selectedStation.id, { geo: { lat, lng } });
      historyStore.push({
        kind: "update",
        before: previousStation,
        after: updated,
      ...
      showToast(error instanceof Error ? error.message : "Failed to set location");
    }
  }
  ...
  async function handleSave(updates: {
    ...
    try {
      isSaving = true;
      const updated = await stationStore.updateStation(selectedStationId, updates);
      historyStore.push({
        kind: "update",
        before: previousStation,
        after: updated,
      ...
      showToast(error instanceof Error ? error.message : "Failed to update station");
    } finally {
      isSaving = false;
    }
  }
  ...
  async function applyOp(op: HistoryOp, direction: "undo" | "redo") {
    ...
    if (op.kind === "create") {
      ...
      if (direction === "undo") {
        ...
        if (selectedStationId === op.station.id) selectedStationId = null;
      } else {
  ...
  async function handleUndo() {
    ...
    try {
      ...
      showToast(error instanceof Error ? error.message : "Failed to undo");
    }
  }
  ...
  async function handleRedo() {
    ...
    try {
      ...
      showToast(error instanceof Error ? error.message : "Failed to redo");
    }
  }
  ...
  async function handleDelete() {
    if (!selectedStationId) return;
    const station = stationState.stations.find((item) => item.id === selectedStationId);
    if (!station) return;
    ...
    try {
      ...
      showToast(error instanceof Error ? error.message : "Failed to delete station");
    } finally {
      isSaving = false;
    }
  }

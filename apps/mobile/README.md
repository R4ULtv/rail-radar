...
the first character, so it works offline. Focusing the field builds the index. A search only
...
stations (up to 3) and saved stations (up to 7). Both lists are stored as JSON files in the app's
